...
fn expand_device_path_topology(c: &mut Criterion) {
    ...
    for &volumes in &[4usize, 64, 256] {
        ...
        // The cast chain is layout-guaranteed: patina's `DevicePath` is
        // repr(transparent) over the raw path bytes, and r_efi's
        // `device_path::Protocol` is a repr(C) 4-byte header (align 1), so
        // viewing the path bytes through it is valid; the consumer
        // immediately reinterprets the pointer as bytes via `try_from_ptr`
        // and nothing writes through the reference.
        unsafe {
