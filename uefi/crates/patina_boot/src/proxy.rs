...
/// Locate the proxy protocol and return a raw pointer to its vtable. Returns
/// [`EfiError::NotFound`] if the proxy driver isn't in the FV, or
/// [`EfiError::Unsupported`] on a major-revision mismatch.
///
/// A raw pointer (not a reference) is deliberate: the vtable is an append-only
/// C struct whose installed length depends on the driver's revision, so the
/// full `Protocol` layout must not be assumed present. Callers read individual
/// fields through the pointer, each guarded by the revision it was added in.
/// The pointer is valid only while boot services are live.
fn locate<B: BootServices>(boot_services: &B) -> Result<*const Protocol, EfiError> {
    use core::ptr;
    ...
    let proxy = ptr as *const Protocol;
    // SAFETY: the proxy always publishes at least `revision` (field 0); reading
    // it does not require the rest of the `Protocol` layout to be present.
    let revision = unsafe { (*proxy).revision };
    if (revision >> 16) != (REVISION >> 16) {
        log::error!(
...
pub fn process_capsules<B: BootServices>(boot_services: &B) -> Result<(), EfiError> {
    ...
    // SAFETY: `proxy` is the raw vtable pointer from `locate()` (non-null, matching major revision); the field is a valid C `efiapi` function pointer installed by the proxy driver.
...
pub fn perform_memory_test<B: BootServices>(boot_services: &B) -> Result<(), EfiError> {
    let proxy = locate(boot_services)?;
    // SAFETY: reading `revision` (field 0) through the raw pointer is always valid; every proxy revision publishes it.
    if unsafe { (*proxy).revision } < 0x0001_0001 {
...
pub fn display_boot_logo<B: BootServices>(boot_services: &B) -> Result<(), EfiError> {
    let proxy = locate(boot_services)?;
    // SAFETY: reading `revision` (field 0) through the raw pointer is always valid; every proxy revision publishes it.
    if unsafe { (*proxy).revision } < 0x0001_0002 {
